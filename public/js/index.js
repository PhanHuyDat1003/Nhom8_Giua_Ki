...
socket.on("draw", ({ myChoice, enemyChoice }) => {
    ...
    setWinningMessage(message, "draw");
    checkGameEnd();
})
...
function setWinningMessage(message, resultType) {
    let p = document.createElement("p");
    p.innerText = message;

    // Thêm class theo kết quả
    if (resultType === "win") {
        p.classList.add("win-message");
    } else if (resultType === "lose") {
        p.classList.add("lose-message");
    } else if (resultType === "draw") {
        p.classList.add("draw-message");
    }
